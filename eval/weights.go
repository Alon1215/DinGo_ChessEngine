...
// Weights holds score for different scenarios
type Weights struct {
	PawnMaterial          Score
...
func (w *Weights) init() {
	// Error: 0.055499
	var autoGeneratedWeights = []int{98, 487, 344, 512, 399, 644, 684, 1668, 1287, 55, 53, 5, 11, 31, 7, 10, 13, 13, 8, 15, 11, 11, 16, 10, 35, 71, 8, -8, 24, 26, 21, 62, -4, -15, -10, -12, -10, 9, 1, 16, 10, 6, 10, 51, -7, 15, -1, 13, 45, 163, -23, -20, -9, -15, 8, 24, 32, -3, -20, 5, -1, -14, 23, 45, 45, 22}
...
func (w *Weights) Apply(weights []int) []int {
	...
	var (
		knightRank  = [8]int{-2, 0, 1, 2, 2, 2, -1, -3}
		knightFile  = [8]int{-2, 0, 1, 2, 2, 1, 0, -2}
		knightLine  = [8]int{-2, 0, 1, 2, 2, 1, 0, -2}
		queenRankMg = [8]int{3, 2, 1, 0, 0, 0, 0, 0}
		queenRankEg = [8]int{0, 1, 2, 3, 3, 2, 1, 0}
		kingRank    = [8]int{3, 1, 0, 0, 0, 0, 0, 0}
		kingFile    = [8]int{2, 3, 0, 0, 0, 0, 3, 2}
		kingLine    = [8]int{-2, 0, 1, 2, 2, 1, 0, -2}
	)
	var (
		kingitPst = wh.nextScore()
		queenPst  = wh.nextScore()
		kingPst   = wh.nextScore()
	)
	...
	for sq := 0; sq < 64; sq++ {
		...
		w.PST[sideWhite][Knight][sq] = Score{
			Mg: (knightFile[f] + knightRank[r]) * kingitPst.Mg,
			Eg: (knightLine[f] + knightLine[r]) * kingitPst.Eg,
		}
		w.PST[sideWhite][Queen][sq] = Score{
			Mg: queenRankMg[r] * queenPst.Mg,
			Eg: queenRankEg[r] * queenPst.Eg,
		}
		w.PST[sideWhite][King][sq] = Score{
			Mg: (kingFile[f] + kingRank[r]) * kingPst.Mg,
			Eg: (kingLine[f] + kingLine[r]) * kingPst.Eg,
		}
	}

	for pieceType := Pawn; pieceType <= King; pieceType++ {
		centerScores(w.PST[sideWhite][pieceType][:])
		for sq := 0; sq < 64; sq++ {
